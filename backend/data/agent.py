...
def run_agent(query: str) -> dict:
    ...
    if intent == "add_task":
        response = ask(f"Extract the task title and description from: '{query}'. Reply in format: TITLE: ... | DESCRIPTION: ...")
        ...
        tasks = get_tasks()
        if not tasks:
            return {"intent": "get_tasks", "response": "You have no tasks yet.", "data": []}
        task_list = "\n".join([f"[{t['id']}] {t['title']} — {t['status']} ({t['priority']})" for t in tasks])
        return {"intent": "get_tasks", "response": f"Here are your tasks:\n{task_list}", "data": tasks}

    elif intent == "complete_task":
        numbers = re.findall(r'\d+', query)
        if numbers:
            result = complete_task(int(numbers[0]))
            return {"intent": "complete_task", "response": f"Task {numbers[0]} marked as done.", "data": result}
        return {"intent": "complete_task", "response": "Please mention the task ID number.", "data": {}}
